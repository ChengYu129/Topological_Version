...
# ╔═╡ 925899f0-5ab3-11ee-0354-f92f30de340d
begin
	using Base.Threads
...
# ╔═╡ 94c00749-cc8c-42cf-b50c-ccd0d97f4814
struct Sofic_Model

label::Vector         
label_explicit::Vector
initial::Vector              
final::Vector
...
function Sofic_Model(label::Vector,label_explicit::Vector,initial::Vector,final::Vector,transition::Dict{String, Any})
	...
	for (i,l) in enumerate(label)
		...
		return new(label,label_explicit,initial,final,transition,typenum,M,CDF,rho,rv,lv)
end
...
# ╔═╡ e0fb75f3-9d2b-4dbd-87c7-50e037fc4a65
function produce_children(model::Sofic_Model, parent_label::String, parent_num::Integer)
...
# ╔═╡ 1b19a299-8c14-446c-8b0e-f14eae714dd4
function run_single_trial(model::Sofic_Model, gen_num)
	is_dieout = false 
	stat = zeros(model.typenum, gen_num); 
...
# ╔═╡ dc36b3a8-6a12-4009-af48-5c25c6be3495
# 創造 0-block code \Phi
function zero_block_code(model::Sofic_Model)
    stat_ratio_explicit = zeros(3,model.typenum)
    stat_ratio_explicit[1, :] = [1 0 0 1 0 0]
	stat_ratio_explicit[2, :] = [0 1 0 0 1 0]
	stat_ratio_explicit[3, :] = [0 0 1 0 0 1]
    return stat_ratio_explicit
end

# ╔═╡ 7cab1515-c4d2-4d1b-a2f9-cdf386339001
function simulate(model::Sofic_Model, gen_num, trial_num)
	count = 0          # number of trial
	dieout_count = 0;  # number of survived trials
	a = sum(model.final)-1
	stat = zeros(model.typenum, gen_num) # model.typenum X gen_num 0-matrix
	#stat_sum = zeros(1,gen_num)
	# stat[i,j] = number of children of type i in j-th generation
    stat_ratio = zeros(model.typenum, gen_num)
	stat_ratio_explicit = zeros(2,model.typenum)
	idential = zeros(1,model.typenum)
	stat_total = zeros(1,gen_num); # 紀錄每一代的總個數
	# theoretical value
    theoretical = zeros(model.typenum, gen_num);
    theoretical[:, 1] = model.initial;
	for gen in 2:gen_num
        theoretical[:, gen] = theoretical[:,gen-1]'* model.M;
    end
	# experirmental value
    ...
    Threads.@threads for trial in 1:trial_num
		...
		stat_sum = zeros(1,gen_num)
		stat_total .+= sum(stat, dims=1)
		stat_sum .+= sum(stat, dims=1)
		stat_ratio .+= stat ./ stat_sum
		stat_ratio_explicit = zero_block_code(model)*stat_ratio
        dieout_count += Int(is_dieout)
		count += 1
    end
	stat_ratio /= count
	stat_ratio_explicit /=count
    stat_total /= count
	return (theoretical,stat_ratio,stat_ratio_explicit, stat_total, dieout_count, count)
end

# ╔═╡ e2f438e9-0f01-409f-b03c-48ebc70f77fe
gr()

# ╔═╡ bc62dc7d-5248-4491-bf3e-451e8bff1f52
#畫圖
function plot_simulation_proportion(model::Sofic_Model, theoretical,stat_ratio,stat_ratio_explicit,stat_total, dieout_count, count, test_type)
	gen_num = size(theoretical, 2)
	p = plot(0:gen_num-1, stat_ratio_explicit', 
		     ...
		     label = hcat(model.label_explicit...),  # always row-vectors...
		     legend = :outertopright #將圖例放置在右上方
	)
G_1=ones(1,gen_num)
G_2=ones(1,gen_num)
G_3=ones(1,gen_num)
G_1[1,:]=0.3333333*G_1[1,:]
G_2[1,:]=0.3333333*G_2[1,:]
G_3[1,:]=0.3333333*G_3[1,:]
plot!(0:gen_num-1, G_1' ,
		 markershape = hcat(repeat([:none])...), 
			 linestyle = hcat(repeat([:dash])...), 
			 linecolor = [i for i in Iterators.take(palette(:tab10), 1)]',
		     label = latexstring("w_1 + w_4"),  # always row-vectors...
		     legend = :outertopright #將圖例放置在右上方
	)
plot!(0:gen_num-1, G_2' ,
		 markershape = hcat(repeat([:none])...), 
			 linestyle = hcat(repeat([:dash])...), 
			 linecolor = [ palette(:tab10)[2]]',
		     label = hcat([latexstring("w_2+w_5")]...),  # always row-vectors...
		     legend = :outertopright #將圖例放置在右上方
	)
plot!(0:gen_num-1, G_3' ,
		 markershape = hcat(repeat([:none])...), 
			 linestyle = hcat(repeat([:dash])...), 
			 linecolor = [ palette(:tab10)[3]]',
		     label = hcat([latexstring("w_3+w_6")]...),  # always row-vectors...
		     legend = :outertopright #將圖例放置在右上方
	)
	xlabel!(latexstring("Generation \$n\$"))
	ylabel!("Distribution Rate")
	xticks!(0:gen_num-1)
	return p
end

# ╔═╡ 8208f5b9-bb84-46a3-8a5e-14ed62c109f9
json_data_topological = """
{
  "label": ["A","B","C","D","E","F"],
  "label_explicit":["0","1/3","2/3"],
  "initial": [0,0,0,0,0,1],
  "final":[1,1,0,0,0,0],
  "transition": {
    "A": [{"num": [0,1,0,0,0,1], "p": 1}],
    "B": [{"num": [0,0,1,1,0,0], "p": 1}],
    "C": [{"num": [1,0,0,0,1,0], "p": 1}],
    "D": [{"num": [0,1,0,0,0,0], "p": 1}],
	"E": [{"num": [0,0,1,0,0,0], "p": 1}],
	"F": [{"num": [1,0,0,0,0,0], "p": 1}]
    }
}"""

# ╔═╡ dbdae0ef-e829-4b0a-b440-cd3d3cce0a53
json_dict_topological = JSON.parse(json_data_topological)

# ╔═╡ f8907213-66b5-4392-aa7d-ab62e633d952
let
model = Sofic_Model(json_dict_topological["label"],json_dict_topological["label_explicit"],json_dict_topological["initial"],json_dict_topological["final"], json_dict_topological["transition"])

for (idx,label) in enumerate(model.label)
	generation = 25
	trials = 300

	model = Sofic_Model(model.label, model.label_explicit, [Int(i==idx) for i in 1:model.typenum],model.final, model.transition)
	
	result = simulate(model, generation, trials)
	print(result)
	#把模擬結果畫出來並存成"pdf"
	savefig(plot_simulation_proportion(model, result..., label), "TEST_model1_proportion_$(label).pdf")
end
...
manifest_format = "2.0"
project_hash = "f5ec125cf607291ab2e43a8a8dffadc92b5ded8a"
